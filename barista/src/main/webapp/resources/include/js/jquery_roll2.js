...
var sample_bn_timeSpeed = 3500; //setInterval 함수 반복실행 시간(해당시간마다 sample_bn_after2펑션을 호출한다.)/*0918 수정*/
var sample_bn_auto = "Y";//자동재생을 설정한다.(Y = 자동재생, N = 효과없음)
function bnr_ac2(){
	var obj = $(".news_bnr2"); //해당 object 변수
	var before_bt = obj.find(".bnr_prev2"); //이전버튼 변수
	var after_bt = obj.find(".bnr_next2"); //다음버튼 변수

	
	after_bt.click(sample_bn_after2);//다음 버튼 클릭시 sample_bn_after2펑션 호출
	before_bt.click(sample_bn_before2);//이전 버튼 클릭시 sample_bn_before2펑션 호출
...
$(document).ready(function(){
	$(".news_bnr2 > div > ul").bind('mouseenter focusin',function(){
		$(".news_bnr2 .bnr_play").css({'display': 'inline'});
		$(".news_bnr2 .bnr_stop").css({'display': 'none'});
		clearInterval(sample_bn_time);
		return false;
	});
	$(".news_bnr2 > div > ul").bind('mouseleave focusout',function(){
		$(".news_bnr2 .bnr_play").css({'display': 'none'});
		$(".news_bnr2 .bnr_stop").css({'display': 'inline'});
		sample_bn_stop();
		sample_bn_time = setInterval(sample_bn_after2,sample_bn_timeSpeed);
		return false;
	});
});
...
function sample_bn_play(){
	...
	sample_bn_time = setInterval(sample_bn_after2,sample_bn_timeSpeed); //해당시간마다 sample_bn_after2펑션을 호출한다.
	return false;
}
//정지 버튼 호출
